...
class mSPoC:
    """Multimodal Source Power Co-modulation algorithm based on :cite:t:`Dahne2013`.

    Given two vector-valued time series X(t), and Y(t), mSPoC finds component pairs
    Sx = Wx.T @ X, and Sy = Wy.T @ Y, such that the covariance between the
    temporally-embedded bandpower of Sx and the time course of Sy is maximized. The
    solution to that optimization problem is captured by the spatial (Wx, Wy), and
    temporal (Wt) filters.

    X(t) must be of shape Ntx x Nx, where Nx is the number of channels and Nt the number
    of time points, and it is band-pass filtered in the frequency band of interest.
    Bandpower of Sx is then approximated by its variance within epochs. The latter are
    defined by the time points of Y(t), which must have a greater sampling rate. Y(t) is
    of shape Nty x Ny, and Nty < Ntx. Both signals are mean-free and temporally aligned.
    ...
    Args:
        ...
        tol (float): Tolerance value used for convergence criterion when comparing
            correlations of consecutive runs.
        ...
        shift_source (bool): Whether to shift the reconstructed sources by the optimal
            time lag found during training. Defaults to True.
    """
    ...
    def fit(
        ...
        """Train mSPoC model on datasets X and Y.

        Implement the pseudo-code of Algorithm 1 of :cite:t:`Dahne2013` for a single
        component pair. After training, the filter attributes Wx, Wy, and Wt are
        updated.
        ...
        # Sort eigenvalues and eigenvectors in descending order and select number of
        # components
        ...
        self.Wx, self.Wy, self.Wt = self.convert_filters_to_DataArray(
            Wx, Wy, Wt, X_full, Y_full
        )
    ...
    def one_unit_algorithm(self, Y, Cxx, Cxxe, tCxxe, time):
        ...
        if not corr_best:
            print(
                f"mSPoC did not converged in any of the {self.N_restarts} restarts!"
    ...
    def validate_inputs_fit(
        ...
        """Validates fit function inputs and returns it with correct dims and labels.
        ...
        if self.Ntx <= self.Nty:
            raise ValueError(
                f"Number of X samples ({self.Ntx}) must be bigger than "
                f"number of Y samples ({self.Nty})"
            )
        if self.Nty < self.N_features:
            raise ValueError(
                f"Number of samples {self.Nty} should be bigger than "
                f"number of features {self.N_features}"
            )
        if self.N_components:
            if self.N_components > self.N_features:
                raise ValueError(
                    f"Number of components {self.N_components} should be smaller than "
                    f"number of features {self.N_features}"
                )
        else:
    ...
    def validate_inputs_transform(
        ...
        """Validates that to-be-transformed data have expected dim. labels and sizes.

        This method ensures that X and Y have the same dimension labels and number of
        features than the ones used during training.
        ...
        if Ntx <= Nty:
            raise ValueError(
                f"Number of X samples ({Ntx}) must be bigger than "
                f"number of Y samples ({Nty})"
            )
        if Nx != self.Nx:
            raise ValueError(
    ...
    def convert_filters_to_DataArray(
        ...
        """Convert filters Wx, Wy, Wt from numpy to DataArray format.

        Checks with that DataArrays have right dimensions and coordinates.
        ...
        Args:
            ...
            Wt (ndarray): Filter matrix for time embedding with shape
                (N_shifts, N_components).
            X (DataArray): DataArray containing the features of modality X.
            Y (DataArray): DataArray containing the features of modality Y.

        Returns:
            tuple: A tuple containing the DataArray versions of Wx, Wy, and Wt.
        """
        ...
        Wt_xr = xr.DataArray(
            Wt,
            coords={
                "time_embedding": self.time_shifts,
...
def temporal_embedding(
    X: np.ndarray, time_shifts: np.ndarray, time: np.ndarray
) -> np.ndarray:
...
def get_orthonormal_matrix(W: np.ndarray) -> np.ndarray:
    """Generate an orthonormal basis for an N-dim. space with basis vectors from W.

    The columns of W are some of the basis vectors.
