...
class SPoC:
    """Source Power Co-modulation (SPoC_lambda) algorithm based on :cite:t:`Dahne2014`.
    ...
    Args:
        ...
        order of scores/eigenvalue. n_comp=1 returns the component of the highest
        eigenvalue. If None, n_comp = Nx, the maximum possible number of components.
    """

    def __init__(self, n_comp: Optional[int] = None):
    ...
    def fit(self, x: cdt.NDTimeSeries, z: xr.DataArray) -> np.ndarray:
        ...
        Args:
            ...
            n_comp (int): Number of components the algorithm will find in decreasing
                order of scores/eigenvalue. n_comp=1 returns the component of the
                highest eigenvalue. If None, n_comp = Nx, the maximum possible number
                of components.
        ...
        if self.n_comp and self.n_comp > self.Nx:
            raise ValueError(
                f"Number of components {self.n_comp} should be smaller than "
                f"number of x channels {self.Nx}"
            )
    ...
    def transform(
        self, x: cdt.NDTimeSeries, get_bandpower: bool = True, Ne: Optional[int] = None
        ...
        If get_bandpower = True, also estimate epoch-wise bandpower of the
        components via the per-epoch variance.

        Args:
            x (:class:`NDTimeSeries`, (channel, time)): Temporal signal
                of shape Nx x Nt.
            get_bandpower: Wether to return only the reconstructed sources
        ...
        if len(x.channel) != self.Nx:
            raise ValueError(
                f"x should have {self.Nx} number of channels, "
                f"but {len(x.channel)} was found!"
            )
        ...
        if get_bandpower:
            ...
            s_power = s_power.rename({"time_bins": "time"}).assign_coords(
                {"time": epochs}
            )
...
def standardize(x: cdt.NDTimeSeries, dim: str = "time"):
