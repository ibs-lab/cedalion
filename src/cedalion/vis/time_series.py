...
class _MAIN_GUI(QtWidgets.QMainWindow):
    ...
    def _init_calc(self):
        ...
        self.snirfData = self.snirfRec.timeseries[
            list(self.snirfRec.timeseries.keys())[0]
        ]

        self.sPos = self.snirfRec.geo2d.sel(
            label=["S" in str(s.values) for s in self.snirfRec.geo2d.label]
        )
        self.dPos = self.snirfRec.geo2d.sel(
            label=["D" in str(s.values) for s in self.snirfRec.geo2d.label]
        )
    ...
    def _draw_timeseries(self):
        ...
        for opt in opt_sel:
            if "S" in opt:
                chan_sel += self.snirfData.source[
                    self.snirfData.source == opt
                ].channel.values.tolist()
                x_opt_sel.append(self.sx[self.slabel == opt])
                y_opt_sel.append(self.sy[self.slabel == opt])
            elif "D" in opt:
                chan_sel += self.snirfData.detector[
                    self.snirfData.detector == opt
                ].channel.values.tolist()
        ...
        for i_ch in range(len(nempty_chan_sel)):
            (self.chan_highlight[i_ch],) = self._optode_ax.plot(
                [x_chan_sel[0][i_ch], x_chan_sel[1][i_ch]],
...
def run_vis(snirfRec: cdc.Recording):
