...
def image_recon_plot(
    ...
    """Render a single view of the head mesh with overlaid scalar data.

    This function creates a new plot handle if one is not provided.
    It plots the specified view (e.g., 'superior', 'left', etc.) onto a given (sub)plot.

    Args:
        X: cdt.NDTimeSeries
            The scalar data per vertex.
        head: TwoSurfaceHeadModel
            Head mesh object that contains attributes (e.g., `.brain` and `.scalp`).
        plotshape: tuple
            The subplot grid shape, e.g., (2, 3) for a 2x3 layout.
        iax: tuple
            The target subplot index (row, col) where this view will be rendered.
        cmap : string, optional
            A matplotlib colormap name or a Colormap object. Default is 'seismic'.
        clim: tuple, optional
            Color limits. If None, they are computed based on the data.
        flag_hbx: str, optional
            Determines which data and surface to use. Options include 'hbo_brain', 'hbr_brain',
            'hbo_scalp', or 'hbr_scalp'.
        view_position: str, optional
            The view direction (e.g., 'superior', 'left', etc.).
        p0: optional
            Plot handle. If None, a new handle is created.
        title_str: str, optional
            Title string to be used on the scalar bar (if applicable).
        off_screen: bool, optional
            Whether to render off screen (useful when saving or creating a GIF).

    Returns:
        A tuple (p0, surf, surf_label)
            p0: the Plotter instance.
            surf: the plotted surface mesh (wrapped in a pyvista object).
            surf_label: text actor (used for labeling, e.g. scale bar) that may be updated later.
    """

    cmap = p.get_cmap(cmap, 1024)
...
def image_recon_multi_view_old(
    ...
    p0,_,surf0_label = image_recon_plot(foo_img, head, subplot_shape, (1,1), clim, view_type, 'scale_bar',
                            None, title_str, off_screen=SAVE )
    p0,surf1,_ = image_recon_plot(foo_img, head, subplot_shape, (0,0), clim, view_type, 'left', p0)
    p0,surf2,_ = image_recon_plot(foo_img, head, subplot_shape, (0,1), clim, view_type, 'superior', p0)
