...
def make_design_matrix(t : xr.DataArray, s, trange : list, idx_basis: str, params_basis, drift_order: int):
    """
    ...
    inputs:
        ...
        idx_basis: string indicating the type of basis function to use ("gaussians", "gamma", "gamma_deriv", "afni_gamma", "individual")
        params_basis: parameters for the basis function (for example for "gaussians" it is [gms, gstd] where 
...
def make_drift_regressors(t, drift_order):
    ...
    for ii in range(1, drift_order + 1):
        ...
        drift_regressors[:, ii, 0] = drift_regressors[:, ii, 0] / drift_regressors[-1, ii, 0]
    ...
    drift_regressors = xr.DataArray(drift_regressors, dims=['time', 'regressor', 'chromo'], coords={'time': t, 'regressor': regressor_names, 'chromo': ['HbO', 'HbR']})
    drift_regressors = drift_regressors.pint.quantify("micromolar")

    return drift_regressors


def make_hrf_regressors(t: xr.DataArray, s: xr.DataArray, trange: list, idx_basis: str, params_basis):
        ...
        for i_conc in range(2):
            for i_ch in range(params_basis.shape[2]):
                ...
                for i_cond in range(n_cond):
                    for b in range(nb):
                        i_c += 1
                        clmn = np.convolve(onset[:, i_cond], np.squeeze(tbasis[i_ch, :, b, i_conc]))
                        clmn = clmn[:nt]
    ...
    hrf_regs = xr.DataArray(a_hrf, dims=['time', 'regressor', 'chromo'], coords={'time': t, 'regressor': regressor_names, 'chromo': ['HbO', 'HbR']})
...
def construct_basis_functions(t_hrf, idx_basis, params_basis):
    ...
    Construct the basis functions
    inputs:
        t_hrf: time vector for the basis functions (in seconds)
        idx_basis: string indicating the type of basis function to use ("gaussians", "gamma", "gamma_deriv", "afni_gamma", "individual")
        params_basis: parameters for the basis function (for example for "gaussians" it is [gms, gstd] where 
                    gms is the mean of the Gaussians and gstd is the standard deviation)
    outputs:
        tbasis: basis functions (time x regressors)
...
def construct_gaussian_basis(t_hrf, params_basis):
    ...
    for b in range(nb):
        tbasis[:, b] = np.exp(-(t_hrf - (trange[0] + (b+1) * gms))**2 / (2 * gstd**2))
...
def construct_modified_gamma_basis(t_hrf, params_basis):
    ...
    for i_conc in range(n_conc):
        ...
        tbasis[:, 0, i_conc] = (np.exp(1) * (t_hrf - tau)**2 / sigma**2) * np.exp(-(t_hrf - tau)**2 / sigma**2)
        ...
        if tt > 0:
            for ii in range(nb):
                foo = np.convolve(tbasis[:, ii, i_conc], np.ones(int(tt / dt))) / int(tt / dt)
...
def construct_modified_gamma_deriv_basis(t_hrf, params_basis):
    ...
    for i_conc in range(n_conc):
        ...
        tbasis[:, 0, i_conc] = (np.exp(1) * (t_hrf - tau)**2 / sigma**2) * np.exp(-(t_hrf - tau)**2 / sigma**2)
        tbasis[:, 1, i_conc] = 2 * np.exp(1) * ((t_hrf - tau) / sigma**2 - (t_hrf - tau)**3 / sigma**4) * np.exp(-(t_hrf - tau)**2 / sigma**2)
...
def construct_afni_gamma_basis(t_hrf, params_basis):
    ...
    for i_conc in range(n_conc):
        ...
        tbasis[:, 0, i_conc] = (t_hrf / (p * q))**p * np.exp(p - t_hrf / q)
        
        if tt > 0:
            foo = np.convolve(tbasis[:, 0, i_conc], np.ones(int(tt / dt))) / int(tt / dt)
...
def get_ss_regressors(y: xr.DataArray, geo3d: xr.DataArray, ssMethod = 'closest' ,ssTresh: float = 1.5, as_xarray: bool = False):
    """
...
def closest_short_channel(y, short_channels, middle_positions, as_xarray = False):
    ...
    if as_xarray:
        closest_short = xr.DataArray(np.zeros((len(y.time), 2, len(y.channel))), dims=["time", "chromo", "channel"], 
...
def max_corr_short_channel(y, short_channels, as_xarray = False):
    ...
    if as_xarray:
        ...
        highest_corr_short = xr.DataArray(ss_data, dims=["time", "chromo", "channel"], 
                                        coords={"time": y.time, "channel": y.channel, "chromo": y.chromo})
        # add regressor dimension
        highest_corr_short = highest_corr_short.assign_coords(regressor = (["channel", "chromo"], channel_iSS))
...
def make_reg_dict(y, add_regressors):
    ...
    # put the data of these short channels in a new xarray
    ss_data = xr.DataArray(np.array(y.sel(channel = unique_short)), dims=["time", "chromo", "regressor"], 
                                    coords={"time": y.time, "regressor": unique_short, "chromo": y.chromo})
    reg_dict = {}
    ...
    for chromo in add_regressors.chromo.values:
        ...
        for ch in np.unique(add_reg_chromo.values):
                reg_dict[chromo][ch] = add_reg_chromo.where(add_reg_chromo == ch, drop=True).channel.values.tolist()
    return reg_dict
