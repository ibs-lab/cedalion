...
def test_normalize_all_alternatives():
    """Test normalization of all alternative landmark names."""
    labels = ["NASION", "INION", "lpa", "rpa", "CZ", "left ear", "vertex", "S1"]
    coords = np.random.rand(len(labels), 3) * 100
    types = [cdc.PointType.LANDMARK] * 7 + [cdc.PointType.SOURCE]
    ...
    expected = {"Nz", "Iz", "LPA", "RPA", "Cz", "S1"}
    assert set(result.label.values) == expected


def test_normalize_duplicate():
    """Test critical duplicate handling: multiple alternatives + canonical present."""
    ...
    # Should keep canonicals, drop alternatives, no duplicates
    assert list(result.label.values).count("Nz") == 1
    assert list(result.label.values).count("Iz") == 1
    assert len(result.label) == 3  # Nz, Iz, S1
...
def test_plot_nonexistent_landmarks_filtered(sample_amp, sample_geo3d):
    ...
    plot_montage3D(
        sample_amp, sample_geo3d, landmarks=["Nz", "NonExistent", "RPA"]
    )
